  ...
  return (
    <div className="p-4 border-y-[1px] border-borderGray">
      ...
      <div className={`flex gap-4 ${type === 'status' && 'flex-col'}`}>
        {/* avatar*/}
        <div
          className={`relative w-10 h-10 rounded-full overflow-hidden ${type === 'status' && 'hidden'}`}>
          <Image path={originalPost.user.img || '/noAvatar.jpg'} alt="" w={100} h={100} tr={true} />
        </div>
        {/* content*/}
        <div className="flex-1 flex flex-col gap-2">
          ...
          <div className="w-full flex justify-between">
            <Link href={`/${originalPost.user.username}`} className="flex gap-4">
              <div
                className={`relative w-10 h-10 rounded-full overflow-hidden ${type !== 'status' && 'hidden'}`}>
                <Image
                  path={originalPost.user.img || '/noAvatar.jpg'}
                  alt=""
                  w={100}
                  h={100}
                  tr={true}
          ...
          {type === 'status' && <span className="text-textGray">15:35 . MAY 5 . 2026</span>}
          <PostInteraction
